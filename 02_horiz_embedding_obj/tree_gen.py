...
def rand_tree(min_children, max_children, num_nodes, balanced=True):
    ...
    # add horiz hybrids
    for layer_id, node_names in g.get_layers().items():
        if len(node_names) <= 1:
            continue

        height = len(node_names)
        max_hybrid = (
            randint(100 * min_hybrid_per_layer_percent, 100 * max_hybrid_per_layer_percent) / 100
        )
        hybrid_edges = 0
        while hybrid_edges / height <= max_hybrid and 2 <= len(node_names):
...
if __name__ == "__main__":
    ...
    min_hybrid_per_layer_percent = 0
    max_hybrid_per_layer_percent = 0.4  # 10 node layer -> 0.4 -> max 4 hybrids
    ...
    trees = []
    for step in sizes:
        trees.append(rand_tree(min_children, max_children, step, balanced=True))
