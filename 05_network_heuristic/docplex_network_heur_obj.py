...
def solve_tree(g: Graph, model_name="y-pos", model_setting=None, export=False) -> List[Graph]:
    ...
    default_model_setting = {
        ...
        "max_dist_gc": 0,  # int, maximize the distance of the center of grandchildren
        "hybrid_cent_lcsa": 0,  # int, lowest common stable ancestor centered to hybrid
        "min_direct_crossing": 2,  # int, minimize direct crossings
    ...
    if model_setting["c_withing_range"]:
        for node_name in g.nodes:
            ...
            children = g.get_children(
                node_name, include_hybrid=(not model_setting["tree_based_embedding"])
            )
            if len(children) == 0:
                continue
    ...
    if model_setting["no_direct_crossing"]:
        for tree_layer in tree_layers.values():
            for node_a_name, node_b_name in combinations(tree_layer, 2):
                ...
                node_a_children = g.get_children(
                    node_a_name, include_hybrid=(not model_setting["tree_based_embedding"])
                )
                node_b_children = g.get_children(
                    node_b_name, include_hybrid=(not model_setting["tree_based_embedding"])
                )
    ...
    ################################################################################################
    """
    Maxmize distance of center of grand-children.
    """
    if model_setting["max_dist_gc"]:
        ...
        for node_name in names:
            ...
            for child_a_name, child_b_name in combinations(children, 2):
                ...
                child_a_children = g.get_children(
                    child_a_name, include_hybrid=(not model_setting["tree_based_embedding"])
                )
                if len(child_a_children) == 0:
                    continue

                child_b_children = g.get_children(
                    child_b_name, include_hybrid=(not model_setting["tree_based_embedding"])
                )
                if len(child_b_children) == 0:
                    continue

                mvar_gchild_a = [model_int_vars[c] for c in child_a_children]
                mvar_gchild_b = [model_int_vars[c] for c in child_b_children]

                objective.append(
                    -1
                    * model_setting["max_dist_gc"]
                    * model.abs(
                        (model.sum(mvar_gchild_a) / len(mvar_gchild_a))
                        - (model.sum(mvar_gchild_b) / len(mvar_gchild_b))
                    )
                )
    ################################################################################################
    """
    Minimise distance of lowest common stable ancestor to hybrid node.
    """
    if model_setting["hybrid_cent_lcsa"]:

        obj_hybrid_cent_lcsa = []

        for hybrid_name in g.get_hybrids():
            lcsa_name = g.get_lcsa(hybrid_name)
            if lcsa_name is None:
                continue
            mvar_lcsa = model_int_vars[lcsa_name]
            mvar_hybr = model_int_vars[hybrid_name]

            obj_hybrid_cent_lcsa.append(mvar_lcsa - mvar_hybr)

        objective.append(model_setting["hybrid_cent_lcsa"] * model.sumsq(obj_hybrid_cent_lcsa))

    ################################################################################################
    """
    Minimise crossing of edges from layer n to n+1.
    Crossing from n to n+k, k ≥ 2 are not affected.
    """
    if model_setting["min_direct_crossing"]:
        ...
        for tree_layer in tree_layers.values():
            for node_a_name, node_b_name in combinations(tree_layer, 2):
                ...
                if model_setting["tree_based_embedding"]:
                    # minimize crossings of hybrid edges as base-tree is modeled by constraints.
                    node_a_children = g.get_hybrid_children(node_a_name)
                    node_b_children = g.get_hybrid_children(node_b_name)
                else:
                    # minimize crossings of all adges
                ...
                for mvar_child_a in mvar_children_a:
                    for mvar_child_b in mvar_children_b:
                        ...
                        """
                        q1 = low(mvar_node_a, mvar_node_b) + low(mvar_child_a, mvar_child_b)
                        q2 = model.abs(
                            low(mvar_node_b, mvar_node_a) + low(mvar_child_b, mvar_child_a)
                        )
